#ifndef LINKED_LIST_INPUT_RESTRICTED_QUEUE_H
#define LINKED_LIST_INPUT_RESTRICTED_QUEUE_H

#include "Generic_node.h"

using namespace generic_node;
template<typename T>
class Linked_list_input_restricted_queue {
private:
    std::unique_ptr<Node<T>> m_head;
    Node<T> *m_tail; //не володіє пам'яттю
    size_t m_size{};
public:
    Linked_list_input_restricted_queue(): m_head{nullptr}, m_tail{nullptr}, m_size{} {}
    Linked_list_input_restricted_queue(const T &data)
        : m_head{std::make_unique<Node<T>>(data)}, m_tail{m_head.get()}, m_size{1} {}
    
    Linked_list_input_restricted_queue(const std::initializer_list<T> &init_list) {
    ...
    friend std::ostream &operator<<(std::ostream &os, const Linked_list_input_restricted_queue &obj) {
        if(!obj.m_head)
            return os;
