...
private:
    ...
    size_t m_size{};
public:
    Linked_list_queue(): m_head{nullptr}, m_tail{nullptr}, m_size{} {}
    Linked_list_queue(const T &data)
        : m_head{std::make_unique<Node<T>>(data)}, m_tail{m_head.get()}, m_size{1} {}
    Linked_list_queue(const std::initializer_list<T> &init_list) {
    ...
    friend std::ostream &operator<<(std::ostream &os, const Linked_list_queue &obj) {
        if(!obj.m_head)
            return os;
